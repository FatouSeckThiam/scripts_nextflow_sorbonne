...
params.inputDir_vcf_converted = "/SCVOL02/run_11/exome/neoepitope_vcf"
//params.outDir = "/SCVOL02/run_09/neoepitope_vcf/GX"
params.outDir = "/SCVOL02/run_11/neoepitope_vcf/GX_with_value"
params.vcf_radia= "/SCVOL02/run_11/vcf/radia"
params.input_Bam_rnaseq="/SCVOL02/run_11/rnaseq/Bam"
params.input_kallisto="/SCVOL01/Workspace/Labreche/Ideation/expression/sleuth_deconv/gene_exp/TSV_up_run_11"
...
samplelist.withReader() {
	...
	kallisto_res = "${params.input_kallisto}/"+kallisto_id+".tsv" 
...
process run_Vep{
	...
	sed -i 's/SYMBOL|Gene/Gene|SYMBOL/' !{id}_wes_rna_intersection.vep.vcf
...
process Vcf_expression_annotator{
	...
	conda run -p /SCVOL01/Tools/pVACtools-1.5.4 vcf-expression-annotator -i id -o ./!{id}_wes_rna_intersection.vep.gx.vcf -s !{id}_Tumour -e expression --ignore-ensembl-id-version !{wes_rna_intersection_vep_vcf} !{params.input_kallisto}/!{id}.tsv custom gene
	
	cp !{id}_wes_rna_intersection.vep.gx.vcf !{params.outDir}
...
process Vcf_readcount_annotator{
	...
	sed -i 's/Gene|SYMBOL/SYMBOL|Gene/' !{wes_rna_intersection_vep_gx_vcf}
	sed -i 's/Gene|SYMBOL/SYMBOL|Gene/' !{id}_wes_rna_intersection.vep.gx.raf.vcf
	cp !{id}_wes_rna_intersection.vep.gx.raf.vcf !{params.outDir}
	"""
}
