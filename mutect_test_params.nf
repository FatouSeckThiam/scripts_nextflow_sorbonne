...
process RunMutect1{
	...
	cpus params.cpus
        ...
        output:
        tuple val(norm), path("./${norm}_call_stats.out"), path("${norm}_filtered.vcf"), path("${norm}_coverage.wig.txt") into mutect1_variants_ch

        shell:
        """
	mkdir -p !{params.outDir}/VC_Mutect1_Strelka2 
        \$JAVA7 -Xmx20g -jar \$MUTECT1 --analysis_type MuTect --reference_sequence !{params.refGenome} --dbsnp !{params.dbsnp} --intervals !{params.intervals} --input_file:normal !{params.inputDir}/!{normal_bam[0]} --input_file:tumor !{params.inputDir}/!{tumor_bam[0]} --out !{params.vcf}/!{norm}_call_stats.out --coverage_file !{params.vcf}/!{norm}_coverage.wig.txt --vcf !{params.vcf}/!{norm}_filtered.vcf
	cp !{params.vcf}/!{norm}_call_stats.out .
	cp !{params.vcf}/!{norm}_filtered.vcf .
	cp !{params.vcf}/!{norm}_coverage.wig.txt .
        cp !{params.vcf}/!{norm}_call_stats.out !{params.outDir}/VC_Mutect1_Strelka2/
        cp !{params.vcf}/!{norm}_filtered.vcf !{params.outDir}/VC_Mutect1_Strelka2/
        cp !{params.vcf}/!{norm}_coverage.wig.txt !{params.outDir}/VC_Mutect1_Strelka2/
        """
 }
...
process Mutect1Filter{
	...
	cpus params.cpus
        ...
        input:
 	tuple val(norm), path(filtered_vcf), path(calll_stats_out), path(tumor_bam) from filterChannel
	//tuple val(norm), path(tumor_bam) from TumorChannel

        output:
	tuple val(norm), file("*_call_stats.pass.out"), file("*_filtered_PASS.vcf") into mutect1_filter_ch

        script:
        """
        grep "#" ${params.vcf}/${norm}_filtered.vcf > ${params.vcf}/${norm}_filtered_PASS.vcf ; grep -v "#" ${params.vcf}/${norm}_filtered.vcf | grep PASS >> ${params.vcf}/${norm}_filtered_PASS.vcf
 	grep KEEP ${params.vcf}/${norm}_call_stats.out >> ${params.vcf}/${norm}_call_stats.pass.out
	
	python /home/seck-thiam/1_metalfox.py -f1 ${params.vcf}/${norm}_call_stats.pass.out -f2 ${params.vcf}/${norm}_filtered_PASS.vcf -f3 ${params.inputDir}/${tumor_bam[0]} > ${params.vcf}/${norm}_filter_stat.report

	cp ${params.vcf}/${norm}_filtered_PASS.vcf .
	cp ${params.vcf}/${norm}_call_stats.pass.out  .
	cp ${params.vcf}/${norm}_filter_stat.report .
        cp ${params.vcf}/${norm}_filtered_PASS.vcf ${params.outDir}/VC_Mutect1_Strelka2/
        cp ${params.vcf}/${norm}_call_stats.pass.out ${params.outDir}/VC_Mutect1_Strelka2/
        cp ${params.vcf}/${norm}_filter_stat.report ${params.outDir}/VC_Mutect1_Strelka2/
